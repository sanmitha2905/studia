...
import { sendWelcomeMessage } from "./messageHandlers.js";
...
const handleRoomOperations = (socket, io, onlineUsers) => {
  ...
  socket.on("join_room", async (data) => {
    try {
      ...
      // Send welcome message if it's the user joining (using a small delay to ensure client is ready)
      setTimeout(() => {
        sendWelcomeMessage(socket, roomId, io);
      }, 1000);
    } catch (error) {
